...
Palintir deployment runs this nightly:

    palintir-backup             # one-shot backup + rotation
"""
...
def create_backup(config: PalintirConfig) -> Path | None:
    ...
    ts = _timestamp()
    snapshot_path = backup_dir / f"palintir-{ts}.db"
...
def rotate_backups(config: PalintirConfig) -> int:
    ...
    backups = sorted(
        list(backup_dir.glob("palintir-*.db"))
        + list(backup_dir.glob("palintir-*.db.gz")),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Palintir database backup")
    parser.add_argument(
        "--no-verify",
        action="store_true",
