"""Configuration loader for Palintir services.
...
# Project root: two levels up from this file (src/palintir/config.py -> Palintir/)
...
class PalintirConfig:
    ...
    auth_token: str = ""
    db_path: str = "/var/lib/palintir/palintir.db"
    enrollment_path: str = "/var/lib/palintir/enrollments"
...
def load_config(environment: str | None = None) -> PalintirConfig:
    """Load and return the full Palintir configuration.

    Args:
        environment: Override environment name (default: from PALINTIR_ENV or "development").
    """
    # Load .env file
    ...
    if environment is None:
        environment = os.environ.get("PALINTIR_ENV", "development")
    ...
    config.auth_token = os.environ.get("PALINTIR_AUTH_TOKEN", "")
    config.db_path = os.environ.get("PALINTIR_DB_PATH", config.db_path)
    config.enrollment_path = os.environ.get("PALINTIR_ENROLLMENT_PATH", config.enrollment_path)
